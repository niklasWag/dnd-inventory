...
> - **R9.2 shipped.** First screen rebuild — Character Sheet in the "Combined" layout, **no feature cull** (every behavior preserved; still renders inside the current Layout — the sidebar nav shell is a later slice).
>   - **Shared components extracted for R9.4 reuse** (user decision this session): the item table (`StashItemsTable`) + the currency display are the single source Storage/Party Stash/Recovered Loot inherit. `StashItemsTable` was already shared — restyled in place (R9 `bg-surface-2` small-caps header, `divide-y`, row hover, right-aligned `tabular-nums`; **new State column** carrying `Badge` chips for Equipped / Attuned / N-of-M charges; rarity → bordered outline `Badge` pill for identified non-common only, via the new `rarityPillClass`). Prop signature unchanged (`stashId`, `characterId?`, `query?`) so all call sites + tests stayed put.
