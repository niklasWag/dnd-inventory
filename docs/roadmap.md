...
- [ ] **R9.3 — Character Sheet (Combined).** _(in progress — 708b110 shipped the shared item-table + currency-panel extraction + `delete-character` UI; the screen BODY still needs the full port to `design-lab/src/character/CharacterCombined.tsx`: framed table card + in-card toolbar (search/category/quick-filter pills), Loadout slot-card rail + slim encumbrance bar, "Purse" currency panel with coin dots.)_ Preserve: ±1 currency + bulk (`+N`/`=N`) edits, ±1 item buttons, equip/attune (+ DM cap-override dialog), split, Pack/Take-out (distinct from Move), CapacityBar, EquippedSlotsPanel, per-tab search. `delete-character` UI (header button + Recovered-Loot cascade confirm — R4.1.b) — DONE.
...
- [ ] **R9.5 — Stash-family screens.** Storage overview (card grid), StorageDetail, Party Stash, Recovered Loot — all reusing the R9.3 shared table + currency display with scope-specific affordances (Banker claim controls, drain, split-evenly) layered on. Transfer modals (Currency + Item move).
...
> - **R9.3 (Character Sheet — Combined) — PARTIAL (commit 708b110).** First screen rebuild, **no feature cull**. _Body still needs the full port to `design-lab/src/character/CharacterCombined.tsx` (framed table card + in-card toolbar, Loadout slot-card rail, "Purse" panel with coin dots) — the extraction + delete-UI below shipped; the visual port is the remaining R9.3 work._
...
>   - **CurrencyRow EVOLVED into the prominent panel** (user decision — not a new wrapper): kept the filename + `CurrencyRow` export + all behaviors (±1 buttons, click-to-edit bulk `+N`/`-N`/`=N`, Convert/Transfer/Split/Drain, full `bankerContext` gating, every `aria-label`). Restyled to the Combined hero: gradient `border-primary/30 bg-gradient-to-br from-primary/10 to-surface shadow-e2` panel with a Coins medallion + big `font-display` gp total on the left, manage actions on the right, and a divided 5-cell cp→pp manager row below. The M4 test's `Total: N gp` string is preserved as an `sr-only` line (the visible total renders separately).
>   - **`delete-character` UI entry (net-new).** The reducer/schema shipped in R4.1.b but had NO UI. New `components/character/DeleteCharacterDialog.tsx` — destructive `AlertDialog` spelling out the OUTLINE §8.3 cascade (items + currency → Recovered Loot; party seat kept). Confirm dispatches via `useDispatch`, toasts, navigates to `/party/:id/settings` (the "create your character" landing, matching Hub's post-delete routing). `useCanDispatch()` disables Confirm in the R5.1.d offline-multi-member window. Gated in the header by the same `canEditCharacter` flag as Edit (owner / DM / solo).
>   - **Layout:** `mx-auto max-w-5xl px-4 py-8`; `font-display` name + identity `Badge` chips (Level / species / class / STR) + Edit / Delete / Rest actions top-right. Tabs → vendored `Tabs` primitive (same 4 tabs). Per non-Storage tab: full-width `CurrencyRow` panel → `lg:grid-cols-[1fr_20rem]` grid: main = search + item table (in a `shadow-e1` card) + Add item; right rail (Inventory only) = `CapacityBar` + `EquippedSlotsPanel`. Storage tab keeps `StorageStashList`.
>   - **Tests:** +9 (`DeleteCharacterDialog.test.tsx` ×3 + CharacterSheet R9.2 ×6 — currency panel gp total, State-column Equipped chip, Inventory rail, delete button gating + confirm copy). Two existing CharacterSheet assertions updated to the new chip structure (identity is now per-field `Badge`s, not one `Level X Species Class` string). web 1051 → 1060. typecheck + lint + build green.
