...
- [ ] **R9.5 — Stash-family screens.** Storage overview (card grid, per-character `/character/:id/stashes`), StorageDetail, Party Stash (`/party-stash`) + Recovered Loot (`/recovered-loot`, party-wide) — all reusing the R9.3 shared `InventoryPanel` + `CurrencyRow` with scope-specific affordances (Banker claim controls, drain, split-evenly) layered on. Replaces the R9.3 `StashPlaceholder`. Transfer modals (Currency + Item move).
