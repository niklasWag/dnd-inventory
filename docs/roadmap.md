...
>
> **BUG-012 follow-up (2026-07-06).** R2.1 widened `itemDefinitionSchema` with `rarity` / `requiresAttunement` / `attunementPrereq` but the `HomebrewForm` UI never grew the matching inputs. Consequence: DMs could not create attunable homebrew magic items — the reducer's `attune` gate rejected every homebrew row with `requiresAttunement !== true`. BUG-012 widens `homebrewDefinitionInputSchema` / `HomebrewDefinitionInput` / `HomebrewDefinitionPatch` / `HOMEBREW_EDITABLE_FIELDS` / `createHomebrew` / `persistCreateHomebrew` / `persistEditHomebrew` in one pass, and adds a `useWatch`-gated section to `HomebrewForm` that surfaces the three inputs when `category === 'magic'` (rarity required via cross-field `.superRefine`). No Prisma migration — the DB columns landed with R2.1.
>
...
- [~] Performance pass on log size (capping, IndexedDB pagination if needed) — skipped for now; revisit when a real slow-log repro surfaces.
- [~] Re-seed conflict hints ("this item has updates" on duplicated PHB/DMG rows) (per `MVP.md` §12) — skipped for now; low daily value until a re-seed schedule exists.
...
- [~] **Multi-party "vault" export** — skipped for now; single-party export/import already round-trips, per-party backup remains a manual N-clicks flow for local mode. Recommended shape when picked up: Hub-level "Export all parties" iterating `listKnownPartyIds()` → JSON-array of envelopes. Inline pointer: `apps/web/src/io/export.ts`, `apps/web/src/io/import.ts`, `packages/shared/src/schemas/exportEnvelope.ts`. (Source: R4.1 followup 2026-06-29.)
...
> - **Docker web slim (2026-07-08).** `Dockerfile.web` runtime switched from `node:22-alpine` + `vite preview` to `nginx:alpine` serving the static bundle. SPA-mode nginx config (`infra/docker/nginx.conf`) handles React Router history-mode fallback and adds sensible cache headers (immutable for `/assets/`, `no-cache` for `/index.html`). Compose port mapping updated to `${WEB_PORT:-5173}:80`; Caddy proxy target updated to `web:80`. Result: **684 MB → 62.8 MB (-91%)**. Base image drop (~170 MB `node:22-alpine` → ~8 MB `nginx:alpine`) plus retiring the workspace `node_modules` from the runtime. Boot verified end-to-end via `docker compose --profile proxy --profile mail up`; web returns 200 from nginx and Caddy proxies cleanly.
> - **Docker server slim — investigated + abandoned.** Explored two shapes: (a) `pnpm --filter @app/server deploy --prod --legacy --no-optional /deploy` pruned tree with the Prisma CLI folded into the runtime image via an isolated `/opt/prisma-cli` install; (b) split into a separate `migrator` init container (Prisma's own [production docs](https://www.prisma.io/docs/guides/deployment/docker) pattern) gated by `service_completed_successfully`. Both produced ~800 MB runtime images. Root cause: Prisma 7's CLI moved former devDeps (`@prisma/dev`, `@prisma/studio-core`, `mysql2`, `postgres`, `effect`, `@electric-sql/pglite`, `chart.js`, `react-dom`) into its core `dependencies`. An isolated install is ~225 MB regardless of `--omit=optional`. On a single-host docker-compose setup the "separate migrator image" pattern just moves the same bytes to another image on the same disk. **Decision: leave `Dockerfile.server` unchanged (~675 MB) until either (a) Prisma 7 minor bumps trim the CLI, or (b) the project moves to a deployment model where migrations run externally (CI-run `prisma migrate deploy` against the target DB before container promotion), at which point the pruned-deploy + no-CLI runtime shape becomes viable.**
...
- [x] ~~**Sync-queue bootstrap pull-after-push test**~~ — **obsoleted by RH1.2 + RH1.3** (see Notes). R3.5 dropped the bootstrap integration test from `apps/web/src/sync/queue.test.ts` because `instanceof BatchRejectedError` checks across `vi.resetModules()` boundaries proved flaky in the existing test rig. Fix: wire module-singleton caching (or replace `instanceof` with structural checks) so the bootstrap pull-after-push + 422 rollback paths get explicit coverage. The happy path + 401 path are tested; the rollback + bootstrap paths are exercised only through the type-checker today. (Source: R3.5 Notes.)
- [x] ~~**Sync-queue unit test for the R4.1.f `isBootstrap` discrimination**~~ — **obsoleted by RH1.2 + RH1.3** (see Notes). `queue.ts:142` now gates `isBootstrap` on `snapshot?.appState == null` in addition to the action type, but the existing 2 queue.test.ts tests don't isolate this branch. The integration test in `apps/server/src/parties/routes.test.ts` exercises the full path end-to-end; a focused unit test asserting `getActivePartyId()` is called (not `'will-be-minted'`) when `appState !== null` would catch a regression at the layer where it would surface. Same flaky-`instanceof` rig as the bootstrap pull-after-push followup above — likely lands together. (Source: R4.1.f.)
- [x] **`GET /sync/state` assertion in the R4.1.f integration test** —The current full-flow test in `apps/server/src/parties/routes.test.ts` asserts the DB rows after user B dispatches `create-character`, but doesn't round-trip through `GET /sync/state?partyId=...` for user B. The mapper layer is well-tested elsewhere, but a focused assertion here would close the seam between the persistor's write side and the state-loader's read side specifically for the post-bootstrap-created character. (Source: R4.1.f.)
- [x] **End-to-end browser tests (Playwright)** —Deferred at M5 per `docs/TECH_STACK.md` §3.3; we're past M5 now. R4.x accumulated two motivating cases — BUG-001 and BUG-002 — where every unit + Vitest server-integration test passed but the real HTTP + real Postgres path failed. Both share a profile: defects that only manifest in the full server-DB-client stack under specific state shapes. R8.4 deliverable: either ship a Playwright rig for that defect profile OR document a further deferral with new criteria. Follow `docs/TECH_STACK.md` §3.5 (climb layers only when a lower-cost layer can't catch the defect). (Source: BUG-001 + BUG-002 postmortems; TECH_STACK §3.3.)

#### R8.4 — Notes

> **R8.4.a shipped 2026-07-09 on `feat/r8-hardening`.** `GET /sync/state?partyId=...` round-trip added to the "joiner creates their character in an existing party" test in `apps/server/src/parties/routes.test.ts`. After the DB-row assertions, the test now injects a state pull as user B and asserts B's character (with correct `ownerUserId`), B's Inventory stash (`scope=character`, `isCarried=true`, correct `ownerCharacterId`), B's `CurrencyHolding` at `cp=0`, and both `create-character` log entries with distinct `actorUserId`s. Closes the seam between persistor write and state-loader read for the post-bootstrap-character path.
>
> **R8.4.b + R8.4.c retired 2026-07-09 as obsoleted.** Both checkboxes were written against a queue shape that no longer exists:
>
> - The "bootstrap pull-after-push" flow was removed entirely by **RH1.3** (2026-07-02). RH1.2 moved id-minting authority to the client (every entity id is a client-minted UUID v7 sent in the action payload; the server echoes it back verbatim in `applied[]`), which eliminated the divergence that made the post-flush `GET /sync/state` re-pull necessary in the first place. RH1.3 then removed the re-pull entirely. The existing "does NOT re-pull /sync/state after acquire" test (`queue.test.ts:127`) already asserts the negative.
> - The `isBootstrap` discriminator gated on `snapshot?.appState == null` (`queue.ts:142` at time of writing) is gone. Post-RH1.2, the queue treats every batch uniformly — there is no bootstrap-vs-non-bootstrap branch left to test.
> - The `instanceof BatchRejectedError` flakiness across `vi.resetModules()` that motivated both items has ALSO gone away structurally: current `queue.test.ts` observes 422 rollback via side effects (log-marker rollback + toast + outbox row removed) rather than `instanceof` checks. Three separate rollback tests exercise the path (BUG-003 rollback, R4.3.a `dm_transfer_target_not_member` rollback, R5.1.c "on 422 removes outbox row"). No test-rig fix needed.
>
> **R8.4.d shipped 2026-07-09 on `feat/r8-hardening`.** Docker-native Playwright rig at `./e2e/`, self-contained (`e2e/docker-compose.yml`) — its own Postgres + server + web + Caddy + mailpit, reusing the **production** `Dockerfile.server` / `Dockerfile.web` / `Caddyfile` so E2E tests what ships. Invoked via `pnpm e2e` / `pnpm e2e:down` / `pnpm e2e:report`. See `e2e/README.md`.
>
> - **Structure (3-layer, Playwright-idiomatic).** `pages/` (readonly locators + interactions, role/label queries, no XPath) → `steps/` (descriptive user actions grouped by screen, each wrapped in `test.step()`, web-first assertions) → `tests/` (prose specs). Mailpit OTP-read helper in `fixtures/`. Locators isolated so markup churn touches one file.
> - **Specs.** `harness` (smoke), `auth-otp-login` (real email-OTP round-trip via mailpit through the SPA), `party-lifecycle` (two browser contexts: create party → join → leave → rejoin → kick). No test-only auth bypass — specs sign in through the real OTP flow.
> - **Secure context via shared netns.** The Playwright container uses `network_mode: service:caddy` so the browser reaches the app at `http://localhost:8080` — mirroring how a real user hits the dev compose's published proxy port. `localhost` is a secure context, so `navigator.locks` (Web Locks API, used by the sync queue) works with no unsafe browser flags. A compose-internal hostname (`caddy:8080`) is an insecure context where `navigator.locks` is `undefined` and the queue flush throws.
> - **Found + fixed BUG-014 during bring-up.** The `party-lifecycle` spec surfaced a real client/server contract mismatch: the client opened the realtime socket during `needsDisplayName` (a regression from BUG-006's fix), which the server rejects, causing a reconnect loop + uncaught `TypeError`. Fixed in `apps/web/src/sync/socket.ts` (connect only when `authenticated`) + `socket.test.ts`. Exactly the BUG-001/002 profile the E2E layer exists to catch. See `docs/BUGS.md` BUG-014.
> - **HTML report.** `retain-on-failure` traces/videos/screenshots bind-mounted to `e2e/playwright-report/` + `e2e/test-results/` (git-ignored); `pnpm e2e:report` opens the trace viewer.
...
- [x] **Dispatch contract** — `store::dispatch` returns `Promise<MutationOutcome>` where `MutationOutcome = { ok: true; applied } | { ok: false; code; message }`. Local-mode resolves synchronously after `set()`; server-mode resolves via the queue. Offline write-block resolves `{ ok: false, code: 'offline_write_blocked' }`; reducer-throw stays a synchronous throw at the raw boundary (see Notes) which `useDispatch` normalizes to `{ ok: false, code: 'reducer_error' }`. (Source: BUG-005; SECURITY §2.1.)
- [x] **Queue outcome-broadcast** — per-dispatch correlation id (UUID v7); `sync/queue.ts` maintains a `Map<dispatchId, resolve>` (`registerOutcome(dispatchId)`) drained on every terminal flush path (200 / 422 / auth / parked / reset). Rejection-code-to-message map extracted from `queue.ts` inline body into `apps/web/src/sync/rejectionToast.ts` (shared with `useDispatch` + the reconnect-drain path). Existing 422 rollback + outbox retry paths unchanged; each now resolves the correlation instead of firing a toast directly.
- [x] **`useDispatch` hook** — `apps/web/src/lib/useDispatch.ts` returns `(action, opts?) => Promise<MutationOutcome>` with `{ onSuccess?, onRejection?, queuedToast? }`. Explicit `toast.loading` + settle (NOT `toast.promise` — our outcome always resolves; see Notes). Default rejection consumer is `useDispatch` itself (every callsite migrated; no orphan programmatic dispatches, so no boot-registered net needed — see Notes).
- [x] **Screen migration** — ~25 mutation-preceded `dispatch(...) + toast.success(...)` callsites migrated to `useDispatch({ onSuccess })`. Batched by area (PartySettings; ItemDetail/ShopDetail/ShopsList; DmDashboard/IdentificationPanel/LootDistributionWizard; CharacterSheet + settings/stash/catalog fields). Non-mutation `toast.success` sites (clipboard copy, file export, direct Dexie wipe, hydrate not dispatch) unchanged.
- [x] **Enforcement** — custom eslint rule `local/no-sync-toast-success-after-dispatch` (`apps/web/eslint-rules/`) flags naive `dispatch(); toast.success()` in the same block scope; allows `useDispatch({ onSuccess })` shape and non-dispatch `toast.success`. Colocated rule test with valid/invalid fixtures.
- [x] **BUG-005 graduation + docs** — moved BUG-005 to `docs/BUGS.md` "Recently fixed" with a closing summary; added the optimistic-UI clarification to `docs/SECURITY.md` §2 intro (optimistic UI includes the "Queued..." intermediate; terminal success signal waits for server ack or local-mode shim resolution).

#### R8.5 — Notes

> **Shipped 2026-07-10 on `feat/r8-hardening`.** Mutation outcome authority — same "one canonical layer per correctness-critical concern" pattern as RH1 (id) / RH2.6 (log) / RH4 (partyId).
>
> - **Reducer-throw stays synchronous (design refinement vs the plan).** The plan text said dispatch would resolve `{ ok: false, code: 'reducer_error' }` for a reducer throw. Implementing that made the store's `dispatch` swallow reducer-invariant throws, which broke ~160 `expect(() => dispatch(...)).toThrow(...)` reducer tests and the `dispatchOrToast` try/catch helpers in `StashItemsTable`. Refinement: the raw `dispatch` still **throws synchronously** on a reducer-invariant violation (a local, deterministic signal unrelated to the BUG-005 async-rejection class); `useDispatch` wraps the call in try/catch and normalizes the throw to `{ ok: false, code: 'reducer_error', message }` so screens get uniform handling. This preserves the `reduce`-throws → `dispatch`-throws test surface AND the existing local error-toast boundaries, while still closing BUG-005 (which is exclusively about the async server-rejection path).
> - **No `toast.promise`.** Sonner's `toast.promise` expects a thenable that *rejects* for its error branch, but a `MutationOutcome` always *resolves* (rejection is `{ ok: false }`, not a thrown promise). `useDispatch` uses an explicit optional `toast.loading(queuedToast)` dismissed on settle + branches on `outcome.ok`. Local mode skips the interim toast (resolves synchronously).
> - **No boot-registered default consumer.** The plan floated a `main.tsx` safety net for orphan dispatches. A grep confirmed every mutation callsite now goes through `useDispatch` (whose default `onRejection` renders the shared toast), so there are no orphans; the net was dropped as dead weight. Programmatic `void dispatch(...)` sites that intentionally own their own error boundary (e.g. `StashItemsTable::dispatchOrToast`) keep their local try/catch.
> - **Outcome resolves even when parked.** A batch that exhausts `MAX_ATTEMPTS` and parks in the outbox resolves `{ ok: false, code: 'sync_paused' }` so an awaiting caller never hangs — the buffered write still persists locally and drains on reconnect. `resetQueue` unwinds any dangling correlations with `{ ok: false, code: 'queue_reset' }`.
> - **`enqueue` now returns `string | null`** (the `dispatchId`, or `null` for the local-only `seed-catalog` short-circuit). The store calls `registerOutcome(dispatchId)` to build the promise it hands back.
> - **Lint fallout.** Changing `dispatch`'s return from `void` to `Promise<MutationOutcome>` surfaced `no-floating-promises` across ~30 test files + a handful of fire-and-forget production sites (`StashItemsTable`, `AddItemModal`, `CatalogPicker`, `CurrencyRow`, `Hub`, `seed.ts`); all got a `void` prefix. The `expect(() => { dispatch(...) }).toThrow()` blocks keep `void` inside the arrow (harmless — the throw fires before `void` applies, and it satisfies the lint).
> - **Test totals.** web 1032 → 1041 (+9: rejectionToast 5, useDispatch 5, queue outcome-correlation 4, minus a couple of reworked write-block assertions). Full workspace typecheck + web lint green.
...
- [x] **R9.0 — Token foundation.** Implement `DESIGN_SYSTEM.md` in `apps/web`: CSS-var tokens (neutrals base, cyan-teal accent, semantic, rarity, elevation, radii) in the global stylesheet + Tailwind config; self-host Cinzel + Inter (ship OFL license files); wire the accent model (default + explicit user pick + opt-in per-class follow with the 12 class colors) and the persistence for the Appearance prefs. No screen changes yet — just the token layer + a smoke test.
- [x] **R9.1 — Primitive vendoring.** `shadcn add` the missing primitives — `card, table, tabs, badge, tooltip, skeleton, sheet` — and apply the R9 tokens to them. These + the existing 10 become the single source for all screen work.
- [x] **R9.2 — Nav shell (sidebar + grouped IA).** The decided CHARTER Option-A container that every party screen renders inside: a left sidebar with the party header + grouped IA (My Character / Party / Reference / DM Tools / footer Settings), a "Hub" back link, active-item highlight, DM-tools gating, collapse-to-icon-rail (persisted), and a mobile bottom-drawer (`sheet`). Sidebar shows only inside `/party/:partyId/*`; auth/Hub/Settings stay chrome-light. **Built before the screen ports** — screens were designed to sit inside this shell.
- [x] **R9.3 — Character Sheet (Combined).** Full port to `design-lab/src/character/CharacterCombined.tsx` (verified against `drawings/character.png`): header (Lv chip + species·class line above the `font-display` name; Add item + Rest + a Character-options kebab top-right), prominent Currency panel (gp-equivalent total + per-denom coin-color dots + inline ±/bulk edits), framed inventory table in an `InventoryPanel` card with an in-card toolbar (search + category select + quick-filter pills), Item/Category/State/Qty/Wt columns with per-row actions collapsed into a kebab `DropdownMenu` (Split, Equip/Attune + DM cap-override, Pack/Take-out, Move, Remove — all preserved), and a right rail (Loadout slot cards + slim encumbrance bar). **IA change:** the old 4 tabs are gone — Inventory is this page; Stashes (per-character) + Party Stash + Recovered Loot (party-wide) are separate sidebar routes (R9.5 screens; placeholder for now). `delete-character` UI shipped (in the kebab).
- [x] **R9.4 — Item Detail (Two-column).** Separate `/item/:id` page. Description/charges (Use/Recharge)/notes left; state toggles + edit + per-item history right. Identify invariant + hint editor.
- [x] **R9.5 — Stash-family screens.** Storage overview (card grid, per-character `/character/:id/stashes`), StorageDetail, Party Stash (`/party-stash`) + Recovered Loot (`/recovered-loot`, party-wide) — all reusing the R9.3 shared `InventoryPanel` + `CurrencyRow` with scope-specific affordances (Banker claim controls, drain, split-evenly) layered on. Replaces the R9.3 `StashPlaceholder`. Transfer modals (Currency + Item move).
- [x] **R9.6 — Catalog + Custom Item + shared ItemPicker.** Catalog Table; Custom Item Editor (single-column, magic-only reveal) as the sole homebrew entry; the shared `ItemPicker` (`layout="list"` default + `layout="rail"` for DM).
- [x] **R9.7 — Shop (Storefront + DM manage).** Both surfaces; ItemPicker rail for add-stock.
- [x] **R9.8 — History (Table).** Filterable audit log; permission-gated rows.
- [x] **R9.9 — DM tools.** DM Dashboard (Command Center + session lifecycle), Hoard Generator (stepper), Loot Distribution Wizard, Identification Panel.
- [x] **R9.10 — Party Settings (Sections).** Members-first; invite/rotate, Banker, kick, DM-transfer, house rules, economy, danger zone; solo-state hiding.
- [x] **R9.11 — Hub + Settings.** Hub Hero (no-gear-pip medallion) as default + List+Detail as the user-selectable alternative; Settings with the **Appearance cluster** (theme / accent / follow-class / Hub-layout).
- [x] **R9.12 — Orphan screens + modals (no dedicated slice).** Surfaces the per-screen slices (R9.3–R9.11) never claimed — either because the design-lab produced no mockup, or the mockup existed but no slice picked it up. Restyle each to the established R9 language for consistency (derive from sibling screens / the chosen mockup where one exists):
  - **Shops list (`ShopsList`, `/shops`)** — the design-lab only shipped the two shop *detail* mockups (Storefront + Manage, done in R9.7), so the shops LIST kept its pre-R9 table. Restyle to the R9 list language (like `StorageOverview` / `CatalogBrowser`): `max-w-6xl`, `font-display` title, framed table/card, status pill, `New shop` primary; keep the DM-full-list vs player-open-only split + create-shop flow.
  - **Auth / login flow (`Login`, `LoginEmail`, `LoginEmailVerify`, `LoginDisplayName`)** — the chrome-light auth screens never got an R9 pass. Bring them to the R9 token/type language (Cinzel display, surface/border tokens, e1/e2 elevation, the `input-otp` primitive styling) as a small coherent set. Unscoped routes, no sidebar; keep the existing OTP + Discord + display-name flows intact.
  - **Character Form (`CharacterForm`, create/edit)** — the design-lab **has** a chosen mockup (`character/CharacterFormGrouped.tsx`; CHARTER 2026-07-13 "Grouped sections") that no slice ported. Restyle to grouped sub-sections (Identity / Class & level / Ability) with helper text, in the Modal shell. Feeds both the Hub create-character dialogs + `EditCharacterDialog`.
  - **Stash + misc modals** — the create/rename/split/pack/convert/drain/split-evenly/add-item/rest-roll modals + the `CatalogPicker` (auto-dispatch picker inside `AddItemModal`) + confirm dialogs (delete stash/character/homebrew, replace-all, edit-character) inherit the R9 palette from the R9.1 vendored primitives, but their internal layout/typography wasn't audited against the `modals/ModalCenteredForm` + `modals/ModalConfirmDestructive` patterns (Cinzel title, icon+title header, field spacing, footer button order). Pass over them for consistency; no behavior change.
  - **Dead-code cleanup** — `stash/StashSearchInput` is now unused (R9.5 replaced its last consumer, StorageDetail, with `InventoryPanel`'s built-in search). Delete it during this slice.
  - **Verified no-restyle-needed** (audited 2026-07-14, listed so they're not re-checked): `PartyScopeGuard` / `PartyScopeSync` / `auth/ProtectedRoute` (routing logic, no UI); `RoleBadge` (token-driven, already R9-consistent); `OfflineBanner` (token-driven status banner — light glance folded into R9.13 polish); `components/ui/*` (vendored primitives, token-driven since R9.1); `inventory/{CapacityBar,EquippedSlotsPanel}` + `stash/CurrencyBreakdown` (shipped inside R9.3/R9.5 hosts).
- [x] **R9.13 — Cross-cutting polish.** Wire `useCanDispatch()` on all primary Save buttons (R5.1.d carryforward); accessibility pass (keyboard nav, ARIA on icon-only buttons, WCAG AA contrast, screen-reader on Character Sheet + Party Stash); optional bulk multi-select on stash tables; empty/loading/error states.
...
> - **R9.0 shipped.** Token layer only — zero screen changes.
>   - **Tokens:** `apps/web/src/index.css` `:root`/`.dark` rewritten from the shadcn Slate/Zinc baseline to the DESIGN_SYSTEM set (Neutral-gray light / Cool-blue-slate dark, cyan-teal accent, rarity band, `--surface`/`--surface-2`, `--shadow-color`, `--radius: 12px`, `--font-sans`/`--font-display`). shadcn-required aliases (`--card`/`--popover`/`--secondary`) retained, mapped onto the R9 surfaces so vendored primitives resolve unchanged. `tailwind.config.ts` gained `surface(.2)`, `rarity.*`, `boxShadow e1/e2/e3`, var-driven `fontFamily.sans/display`, and the 12px radius scale (`md` now `-4px`, `xl`=`lg`=`--radius`). **Radius shift is intentional** (0.5rem → 12px) — existing primitives look slightly rounder now; that's the R9 baseline.
>   - **Fonts self-hosted via Fontsource** (`@fontsource-variable/inter/wght.css` + `@fontsource/cinzel/{500,600,700}.css`, imported in `main.tsx`) — chosen over a Google `<link>` for offline-first correctness (local-first app), privacy (no per-load IP to Google), and OFL bundling. Vite emits the `.woff2` into `dist/assets/` (verified via `build`); no runtime CDN call, no font binaries in git. Declared families: `'Inter Variable'`, `'Cinzel'`.
>   - **Accent model** = two standalone Zustand stores mirroring `theme.ts` (UX chrome, kept OUT of the reducer so party hydrate/wipe never touch it). `store/accentColors.ts` holds the 6 presets + 12 class colors (`classColorFor` matches `Character.class` case-insensitively, `undefined` for homebrew → falls back to preset). `store/accent.ts` persists `accent` + `accentFollowClass` to Dexie `meta`; `resolveAccent(ctx, theme)` applies follow-class only when ON + in a party (`activeClass` non-null) + class known, else the explicit preset. `attachAccentSideEffects()` subscribes to accent+theme+main stores and writes `--primary/--accent/--ring` (+`-foreground`) onto `:root`; wired in `main.tsx` boot (prefs hydrated pre-render to avoid a flash; side-effect attached post-store-hydration so first paint reflects the active character's class). **No Settings UI yet** — the Appearance cluster (picker + follow-class toggle) lands in R9.10.
>   - **Tests:** +10 (`accent.test.ts`) — store defaults/persist/hydrate/garbage-fallback + `resolveAccent` matrix (follow off/on, in/out of party, homebrew, case-insensitive). web 1041 → 1051. typecheck + lint + build green.
> - **R9.1 shipped.** Vendored the 7 missing shadcn primitives (`card, table, tabs, badge, tooltip, skeleton, sheet`) via `shadcn add` → 17 total in `src/components/ui/`. Added `@radix-ui/react-tabs` + `@radix-ui/react-tooltip` (sheet reuses the existing `react-dialog`). **No hand-editing** (CLAUDE.md): the primitives are token-driven so they inherit the R9 palette automatically — every class they reference (`bg-card/primary/secondary/destructive`, `*-foreground`, `border`, `ring-ring`, `text-muted-foreground`) already exists in the R9 token set. R9 elevation (`shadow-e1/e2/e3`) is applied by CONSUMERS via `className`, not baked into the primitive (card stays stock `shadow-sm`). CLI quirk: it wrote to a literal `@/` dir instead of resolving the alias — moved the 7 files into `src/components/ui/` + removed the stray dir. `index.css`/`tailwind.config.ts` untouched by the CLI. Prettier-formatted the 7 (matches the existing-primitive convention; eslint `ui/**` override only relaxes `no-explicit-any`). typecheck + lint + 1051 tests green.
> - **R9 RE-SEQUENCED (2026-07-13).** After the first Character-Sheet build the user found it "nothing like the mockups." Root cause: (1) the decided **nav shell (sidebar) was never built**, so screens rendered naked with no framing; (2) the screen body was reconstructed from the prose specs instead of the **`design-lab/` mockup .tsx files** (the real visual ground truth — git-excluded, so easy to forget). Fix: inserted a **new R9.2 = Nav shell** before the screens; the former R9.2 Character Sheet became **R9.3** (extraction shipped; body still needs the full mockup port); everything shifted down one (old R9.11 → R9.12). Standing rule now: **read `design-lab/src/**` as the visual contract for every R9 slice.** Token layer was audited and matches the lab + `DESIGN_SYSTEM.md` exactly — the gap was screen execution, not the foundation.
> - **R9.2 shipped (Nav shell — sidebar).** The CHARTER Option-A container built BEFORE the screen ports. New `components/nav/Sidebar.tsx` (+ `store/sidebar.ts` collapse pref, Dexie-meta-persisted, mirroring theme/accent): party header (name + member count + settings gear), a **"Hub" back link** (Home icon; the earlier inline "Switch" was dropped as ugly), grouped IA (My Character / Party / Reference / DM-Tools-gated / footer Settings), active highlight via `NavLink aria-current`, collapse-to-icon-rail with `tooltip` labels. `Layout.tsx` rebuilt into a two-shape shell: inside `/party/:partyId/*` (+ AppState) → sidebar frames the `<Outlet/>` (desktop rail + mobile hamburger→`Sheet` drawer, "the drawer IS the sidebar"); unscoped auth/Hub/Settings → chrome-light (no sidebar). Session badge + server-mode logout moved into the party shell. `Layout.test.tsx` rewritten to assert shell shape + badge (nav items now covered by `Sidebar.test.tsx`). Reference: `drawings/nav-sidebar.png` (sidebar only; rest of that shot is outdated). **Tests:** +20 net (`sidebar.test.ts` ×5, `Sidebar.test.tsx` ×15). web 1060 → 1071. typecheck + lint + build green.
> - **R9.3 shipped (Character Sheet — Combined).** Full port to `design-lab/src/character/CharacterCombined.tsx`, verified against `drawings/character.png`. **No feature cull.** Two commits: 708b110 (extraction + delete-UI) + the Inventory-only rebuild.
>   - **IA change — tabs → separate pages.** The old 4-tab sheet is gone. The Character Sheet is now **Inventory-only**; Stashes (per-character `/character/:id/stashes`), Party Stash (`/party-stash`), Recovered Loot (`/recovered-loot`, party-wide) are separate **sidebar** destinations (kebab-case routes; per-character nested under `:characterId`). Their R9.5 screens aren't built yet → a shared `StashPlaceholder` ("coming in R9.5") holds those routes. Selector slimmed (dropped tab/banker-context state); `searchByTab` retired (per-tab isolation is now automatic — each surface is its own page).
>   - **`InventoryPanel` (new shared component)** — the framed `shadow-e1` table card + in-card toolbar (search input + shadcn `Select` category filter with capitalized names + quick-filter pills All/Equipped/Attuned/Unidentified), over the shared `StashItemsTable`. R9.5 stash screens reuse it. Toolbar controls normalized to `h-9`. Owns the search/category/state filter state; `StashItemsTable` gained additive `categoryFilter` + `stateFilter` props (no-op when unset) so it stays scope-agnostic.
>   - **Shared components extracted for R9.5 reuse** (user decision this session): the item table (`StashItemsTable`) + the currency display are the single source Storage/Party Stash/Recovered Loot inherit. `StashItemsTable` was already shared — restyled in place (R9 `bg-surface-2` small-caps header, `divide-y`, row hover, right-aligned `tabular-nums`; **new State column** carrying `Badge` chips for Equipped / Attuned / N-of-M charges; rarity → bordered outline `Badge` pill for identified non-common only, via the new `rarityPillClass`). Prop signature unchanged (`stashId`, `characterId?`, `query?`) so all call sites + tests stayed put.
>   - **CurrencyRow → the prominent panel** (evolved, not wrapped): kept the filename + `CurrencyRow` export + all behaviors (±1 buttons, click-to-edit bulk `+N`/`-N`/`=N`, Convert/Transfer/Split/Drain, full `bankerContext` gating, every `aria-label`). Final look matches the mockup: gradient `border-primary/30 … shadow-e2` panel, Coins medallion + "Currency" eyebrow + `font-display text-xl` **gp-equivalent** total (172.21-style, formatted to ≤2 decimals), and a divided 5-cell cp→pp row where each cell is **coin-color dot + label / big `font-display` value / a `− +` button pill** (value above the buttons, not between them). The M4 test's `Total: N gp` string kept as an `sr-only` line. (Label stayed "Currency" per user, though the mockup says "Purse".)
>   - **Row actions → per-row kebab `DropdownMenu`** (user request — declutter but keep every action): Split, Equip/Unequip + Attune/Unattune (dynamic-label toggles, accent-tinted when the flag is on) + DM cap-override dialog, Pack, Take out, Move, Remove. Inline ± qty steppers stay in the Qty column (small `h-5` ghost buttons). Added a **Wt** column. Header alignment fixed (Qty/Actions centered over their cells, Wt right); category cells + Select options `capitalize`d.
>   - **`delete-character` UI entry (net-new).** New `components/character/DeleteCharacterDialog.tsx` — destructive `AlertDialog` spelling out the OUTLINE §8.3 cascade (items + currency → Recovered Loot; party seat kept). Confirm dispatches via `useDispatch`, toasts, navigates to `/party/:id/settings`. `useCanDispatch()` disables Confirm in the R5.1.d offline window. Now lives in the header **"Character options" kebab** (alongside Edit character), gated by `canEditCharacter` (owner / DM / solo).
>   - **Layout:** `mx-auto max-w-6xl px-4 py-8`; header = `Lv` accent chip + "species · class · STR" line above the `font-display text-3xl` name; top-right = **Add item** (primary) + **Rest** + the Character-options kebab. Body = full-width `CurrencyRow` → `lg:grid-cols-[1fr_20rem]`: main = `InventoryPanel`; right rail (`lg:sticky`) = `EquippedSlotsPanel` (Loadout slot cards) + `CapacityBar` (slim custom encumbrance bar). Tabs removed entirely.
>   - **Tests:** `StashItemsTable.test.tsx` updated to open the per-row menu + query `menuitem`s (16 tests reworked; `aria-disabled` for disabled items). CharacterSheet tests updated for the Inventory-only layout + header kebab (Edit/Delete visibility now checks the "Character options" trigger; equip via the row menu). Storage-tab tests removed (coverage lives in `StorageStashList.test.tsx`; note added). web suite 1072 green; typecheck + lint + build green.
> - **R9.4 shipped (Item Detail — Two-column).** Re-skin of the existing `screens/ItemDetail.tsx` into `design-lab/src/item/ItemDetailTwoColumn.tsx` (verified against `drawings/item-details.png`). Not a rebuild — the separate `/item/:id` page + Two-column layout were already the locked CHARTER decision (2026-07-13); every prior behavior stayed wired. **No feature cull.**
>   - **Layout.** `max-w-5xl` page; header = `Package` category · stash eyebrow, `font-display text-3xl` name (italic-muted "Unknown Magic Item" when unidentified), rarity pill (R9 `rarityPillClass`, identified non-common only) + primary "Requires attunement" pill w/ inline prereq, right-side Qty/Weight. Body = `lg:grid-cols-[1fr_18rem]`: LEFT `<main>` = unidentified DM-hint dashed block → Description card → Charges card → Notes card; RIGHT sticky `<aside>` = State card → History card. The History card wraps the shared `ItemHistory`, which was restyled to the R9 language: a leading `ShieldCheck` icon on the "History" heading + the show-all control flipped from a checkbox to a text button that toggles between "Show all events (+N)" (`ArrowLeftRight`) and "Ownership only" (`Eye`). The toggle renders **only when it would change what's shown** (the ownership-only filter is hiding non-transition events, or the user has expanded) — suppressed for items whose events are all ownership-transitions, since there's nothing to collapse. `ItemHistory` is only consumed by ItemDetail, so the header change is contained.
>   - **Net-new: Equipped + Attuned State toggles** (were only in the Character-Sheet row kebab before). Reuse the StashItemsTable plumbing verbatim: `dispatchMintingAction` for equip/attune (BUG-008 auto-split), plain `useDispatch` for unequip/unattune, `attunement.hasFreeSlot` pre-disable for the Attune direction (non-DM), and the R4.5 DM/solo cap-override `AlertDialog` (`isCurrentUserDmOrSolo`). Toggles show only for carried-Inventory items; else a "available only in a carried Inventory" note. The **Attuned toggle is further gated on `def.requiresAttunement`** (non-attunement gear shows only Equipped — the reducer would reject an attune anyway). The `Identified` toggle (scope-independent) rounds out the card.
>   - **Hint editor kept, restyled.** The `identify`-dispatching hint input + Save/Clear moved into the left-column DM-hint block (shown while unidentified); label is `sr-only` "Unidentified hint (DM only)" so the existing tests still resolve it. An `sr-only` "Unidentified" marker preserves the a11y signal + identify-invariant tests since the mockup drops the visible badge (italic-muted name carries the state).
>   - **Notes/custom-name edit** collapsed behind a Notes-card "Edit" (Pencil) affordance; the `react-hook-form` + Zod `edit-item-instance` flow is unchanged, a successful save closes the editor. Charges card splits the big `n / max` counter (labelled "Charges") from the recharge-rule caption in the header (was one `formatChargesLong` string) — the R2.2.1 inline roll input is preserved.
>   - **Dead code culled.** Dropped the old cost/definition/quantity key-value grid + the "adjust quantity from the stash table" footnote + the standalone Identification `<section>` + the unused `formatChargesLong`/`rarityClasses`/`ShieldCheck` imports; kept every load-bearing dev note (R2.2/R2.2.1/R2.3/R4.5/BUG-008/M2.5) at its new home.
>   - **Tests.** 5 existing tests updated for the new Notes-card Edit gate (open the editor first) + the split charges display; +6 net-new for the State toggles (equip, unequip, attune, cap-override dialog, Attuned-hidden-without-attunement, hidden-outside-Inventory). `ItemHistory.test.tsx` show-all selectors flipped checkbox → button, +1 for toggle-hidden-when-nothing-to-collapse. `ItemDetail.test.tsx` 29 → 35; web suite 1072 → 1079. typecheck + lint green.
> - **R9.5 shipped (Stash-family screens).** Built the four stash-family surfaces the R9.3 sidebar routed to, replacing the shared `StashPlaceholder` (deleted). Reference: `design-lab/src/storage/StorageOverviewCards.tsx` + `transfer/{TransferItemMove,TransferCurrency}.tsx`, verified against `drawings/storage-list.png`. **No feature cull** — every pre-R9 stash behavior kept.
>   - **StorageOverview (new screen, `/character/:id/stashes`).** Screen port of the old M3 `StorageStashList` component (deleted): `max-w-3xl` card grid (`sm:grid-cols-2`), each card a `Package` medallion + name + item-count (sum of quantities) + `CurrencyBreakdown` + `ChevronRight`, a "New Storage stash" CTA (`CreateStashModal`), empty state, back-to-character link. Resolves the character from the URL `:id` instead of a prop. Coverage moved to `screens/StorageOverview.test.tsx` (10 tests, ported from `StorageStashList.test.tsx`).
>   - **StorageDetail restyle.** Swapped the bare `StashSearchInput` + `StashItemsTable` + "Items" header for the shared **`InventoryPanel`** (framed card + in-card search/category/quick-filter toolbar) so Storage reads identically to Inventory; `font-display` header, `space-y-6 px-4 py-8`. Panel owns its own search state (retired the per-screen `query`). No `characterId` → no equip/attune. All 10 StorageDetail tests stayed green.
>   - **Party Stash + Recovered Loot (new `screens/SharedPools.tsx`).** One shared `SharedPoolScreen` + two exports (`PartyStash` at `/party-stash`, `RecoveredLoot` at `/recovered-loot`). Rebuilt the R4.2.e **banker-context** builder that was lost when R9.3 dropped the CharacterSheet tabs (from git `708b110`): `userIsBanker`/`userIsDmWithBankerActive`/`userIsGatedFromPool`/`isPartyStash` (§3.14 — banker from `Party.bankerUserId`, DM barred from banker). Each screen = header + `CurrencyRow` (with banker context) + `InventoryPanel` (no `characterId`) + Add item. Party-scope stash resolved by `scope==='party'`; Recovered Loot by `party.recoveredLootStashId`. Item "claim" stays the existing row Move action. +3 screen-level tests (`SharedPools.test.tsx`); the per-flag CurrencyRow gating is covered exhaustively in `CurrencyRow.test.tsx`.
>   - **Transfer modals restyled** to the mockups (behavior unchanged): `MoveItemModal` gained a `Package` title icon + a pre-selected item summary card (name + source · ×N in stack) + a source→target `ArrowRight` row; `CurrencyTransferModal` gained a `Coins` title icon + a source→target row. Kept the richer gp-equivalent status line (better than the mockup). All modal tests green.
>   - **Routes + cleanup.** `router/index.tsx` swapped the 3 `StashPlaceholder` routes for the real screens + updated the route-map doc; `StashPlaceholder.tsx` + `StorageStashList.{tsx,test.tsx}` deleted (dead). CharacterSheet.test breadcrumb repointed at `StorageOverview.test.tsx`. web suite 1079 → 1084; typecheck + lint green.
> - **R9.6 shipped (Catalog + Custom Item + shared ItemPicker).** Reference screens restyled to the R9 language. Ref: `design-lab/src/catalog/{CatalogTable,CustomItemSingle,ItemPickerList,ItemPickerFilterRail}.tsx`, verified against `drawings/catalog.png`. **No feature cull.**
>   - **CatalogBrowser → R9 table.** `max-w-6xl` (baseline), "Reference · shared catalog" eyebrow + `font-display` title, framed in-card filter bar (`shadow-e1`, `bg-surface-2` controls) keeping ALL five filters (search + category + rarity + source + attunement), framed table card (`bg-surface-2` small-caps header, `divide-y`, row hover). Rarity moved from its own column to an **inline pill** (R9 `rarityPillClass`) beside the name, with `Sparkles` "attune" + `FlaskConical` "Homebrew" tags. **Kept** New-homebrew + per-row Duplicate/Edit/Delete; **no per-row Add** (Catalog is a reference/homebrew screen — "add to which stash?" is ambiguous; adding stays in the stash Add-item flow). One test updated (rarity pill now shares the name cell → locate row by name text).
>   - **HomebrewForm → single-column (CustomItemSingle).** `FlaskConical` title icon, `max-w-md` scrollable body, single-column fields (Name → Category → Weight+Cost row), magic-only block restyled to the accent-tinted `border-primary/30 bg-primary/5` "Magic properties" card. Currency select folded next to Cost with an `aria-label="Currency"` (kept the label-text query working). All Zod + BUG-012 refinement + modal/inline variants + dev notes preserved; 17 tests green.
>   - **ItemPicker → list + rail.** Restyled to the List mockup (rarity **dot bar** + name + `source · category · rarity · price` subline + `Sparkles`/`FlaskConical` icons) with an **interactive rarity filter** (chips in `list`, a left rarity+category **rail** in the new `layout="rail"`). New `layout?: 'list' | 'rail'` prop (default `list`). A locked `rarityFilter` prop (Loot Wizard row hint) still wins + hides the interactive filter. Callback-only `onPick`/`onCancel` API unchanged; consumers (Loot Wizard, Shop add-stock) untouched (default list). Rail wiring into Shop add-stock lands in R9.7. +5 net-new direct tests (`ItemPicker.test.tsx` — previously only consumer-tested).
>   - web suite 1084 → 1089; typecheck + lint green.
> - **R9.7 shipped (Shop — Storefront + DM manage).** `ShopDetail` split into two role-branched surfaces (ports of `design-lab/src/shops/{ShopStorefront,ShopManage}.tsx`, verified against `drawings/shop-{player,dm}.png`). **No feature cull** — every prior buy/sell/stock/settings behavior kept; the reducer-backed `edit-shop` price-modifier + sell-rate edits are now surfaced (net-new UI on an existing action).
>   - **Storefront (player).** Gradient banner (`Store` medallion + name + open/closed pill + `Buy ×mod` / `Sells at %` chips + `Sell items` button) over an "On the shelves" stock **card grid** (`sm:2/lg:3`); each card = rarity pill + N-in-stock + price + a qty **stepper** + `Buy N` (dispatches `purchase` with the stepped quantity). Empty-shelves state.
>   - **Sell modal.** Opened from the banner; search-filtered inventory list (sellable = items with a catalog cost, since `sale` throws otherwise) + per-row qty stepper + `Sell N` (dispatches `sale`). Replaces the old inline sell panel; header shows the merchant sell-rate %.
>   - **DM manage.** "Manage shop" header, **Shop settings** panel (`Settings2` header; storefront-status `role="switch"` toggle via `set-shop-open`; **editable Price modifier + Sell rate** committing on blur via `edit-shop`), a **Stock table** (Item / Rarity pill / Qty / Buy price + per-row trash) with an add-stock row whose item comes from the **`ItemPicker` rail** (`layout="rail"`; default-price prefills the override placeholder, BUG-013 no-cost guard kept), and a **Danger zone** (delete-shop `AlertDialog`).
>   - **DM/solo view toggle.** Since a solo user is DM-or-solo but still needs to buy/sell, DM/solo gets a **Manage ⇄ Storefront** segmented toggle (default Manage) so they can flip to the player storefront. Players never see the toggle. Resolves the "solo can't buy from the manage view" gap.
>   - **Tests.** Rewrote the ShopDetail suite for the split (toggle-switch status, DM-buys-via-storefront, add-stock rail, override placeholder, no-price disabled Buy) + 3 net-new (storefront qty-stepper Buy N, Sell modal, price-modifier edit-on-blur). web suite 1089 → 1092; typecheck + lint green.
> - **R9.8 shipped (History — Table).** Restyled `HistoryScreen` to the `HistoryTable` mockup (verified against `drawings/history.png`): `max-w-5xl`, party-name eyebrow + `font-display` "Audit Log" title, and the flat entry list converted to a **framed table** (When / Actor+role badge / Type chip / Summary; `bg-surface-2` small-caps header, `divide-y`, row hover). **No feature cull** — all 5 rich filters (Session / Character / Item / Actor-role selects + the action-type checkbox group), pagination ("Load more"), and the §3.4 permission gate (`canSeeLogEntry`) are unchanged.
>   - **Type chips.** A `typeVisual()` helper groups the 40+ log types into families (Acquire / Consume / Transfer / Currency / Identify / Loadout / Charges / Character / Session / Party) sharing an icon + token-driven chip color, with a neutral "Other" fallback — purely presentational; filtering still keys off the exact `entry.type`.
>   - **Net-new free-text search.** A search box (from the mockup) filters over the rendered summary + resolved actor label, applied last in the `applyFilters` pipeline so the structured filters narrow first.
>   - **Tests.** Converted the list→table (`listitem` queries → a `bodyRows()` helper that slices the header off `getAllByRole('row')`); +1 net-new search test. `HistoryScreen.test.tsx` 12 → 13; web suite 1092 → 1093; typecheck + lint green.
> - **R9.9 shipped (DM tools).** All four DM screens restyled to the R9 language (refs: `design-lab/src/dm/DmCommandCenter.tsx`, `hoard/HoardGenStepper.tsx`, `loot/LootStepper.tsx`, `drawings/dm-panel.png`). **No feature cull** — every session/roll/distribute/identify dispatch path preserved. Single R9.9 commit.
>   - **DM Dashboard → Command Center.** `DmDashboard` restyled to the mockup: party-name eyebrow + `font-display` "DM Command Center", a current-session gradient banner (session number/title + a "Session tools"/"Start session" button anchoring to the Sessions region), a 4-tile DM-tool launcher grid (Generate hoard → `/loot/generate`, Distribute loot → `/loot/distribute`, Identify → `/identify`, Manage party → `/settings`), and a "Party at a glance" list + Total-party-gold / Party-Stash / Recovered-Loot side cards. The full `SessionsSection` + `SessionRow` lifecycle (start/end/notes via `start-game-session`/`end-game-session`/`edit-game-session-notes`, `useCanDispatch` gating, end-confirm dialog) is preserved below the fold (`id="sessions"`). `DmOnlyRoute` unchanged. `role="region"` aria-labels kept (via `<section aria-label>`) so the existing tests resolve.
>   - **Hoard Generator → stepper.** `HoardGenerator` restyled from a flat form to the `HoardGenStepper` 3-step flow (Parameters → Review roll → Hand off) sharing the step-indicator + bordered-card + footer-nav shell. `hoard.rollHoard` + the route-state handoff to the wizard preserved. Tests walk the stepper (Roll → Next → Continue).
>   - **Loot Distribution Wizard → stepper** (subagent-ported). Flat table → the `LootStepper` 3-step flow (Review hoard → Assign targets → Confirm) with the shared shell, unassigned-row gating + warning, and grouped confirm summary. `distribute()` (R8.5 batched dispatch + continue-on-failure), `buildRowsFromRoll`, the row model, and the full `ItemPicker` wiring preserved verbatim. Heading now "Distribution wizard". Tests updated to walk the stepper.
>   - **Identification Panel → R9 cards** (subagent-ported; no dedicated mockup — derived from Catalog/History/Shop siblings). Framed section cards (`shadow-e1`, small-caps card headers), `font-display` title + "DM tools · Identification" eyebrow, `max-w-5xl`, rarity chips swapped to the token-driven `rarityPillClass`. All `identify` / `identify-batch` / hint-editing dispatch + the batch dialog preserved byte-for-byte; no test changes needed.
>   - **Tests.** DmDashboard 19 (class assertion loosened to substring), HoardGenerator 5 (stepper-walk helpers), LootDistributionWizard 6 (stepper-walk + div-layout row counts), IdentificationPanel 6 (unchanged). web suite stayed 1093; typecheck + lint green.
> - **R9.10 shipped (Party Settings — Sections).** `PartySettings` restyled to the `PartySettingsSections` mockup: a local `Section` helper (framed card, `font-display` uppercase title + description, `danger` variant for the destructive zone) reproduces the design-lab `settings/kit` look with app tokens (no design-lab import). Header = `Shield` "Party settings" eyebrow + `font-display` party name. **Members-first IA**: in server mode the section order is **Members → Invite code → Names → House rules → Economy → (Create character) → Danger zone (Leave)**; member rows restyled to avatar + name + role/Banker badges + Make-Banker/Transfer-DM/Kick controls. Local mode (no server member list) shows Names → House rules → Economy → Create-character only. **No feature cull** — every dispatch (rename, appoint/revoke Banker, kick, dm-transfer, invite rotate/copy, encumbrance, economy, leave, create-character) + `useCanDispatch` gating + all confirm dialogs + solo-hiding + R4.2/R4.3/R6.1/BUG-011 dev notes preserved. Width kept at `max-w-3xl` (settings-form tier). The screen's Back button was dropped (it's a sidebar destination, not a detail route). Section titles stay `<h2>` + `aria-label` so the 8 existing tests resolve unchanged. web suite 1093; typecheck + lint green. **(Done directly in the main thread — a subagent's first pass kept the original field order instead of members-first and was reverted.)**
> - **R9.11 shipped (Hub + Settings).** Two commits, both done directly in the main thread. **R9.11a (Settings)** — restyled `Settings` to the `SettingsProfileHero` mockup: back-nav button (no sidebar on this unscoped route), a profile hero (server mode → initial-avatar + display name + email + version; local mode → plain title), and framed `Section`/`Row` card helpers (mirrors the R9.10 PartySettings kit). Section order **Account → Login methods → Appearance → Sessions → Backup → (Corrupted party) → Danger zone**. New **Appearance cluster**: `ThemeField` rewritten from a `<select>` to a segmented button-group radiogroup (Light/Dark/System); new `AppearanceFields` = `AccentField` (swatch radiogroup over `accentPresets`), `FollowClassField` (role="switch" → `accent.setFollowClass`), `HubLayoutField` (select → new `hubLayout` store). `LinkedAccounts` gained green "Connected" badges. New `store/hubLayout.ts` (`'hero' | 'list'`, default `'hero'`, Dexie-`meta`-persisted, mirrors `sidebar.ts`; hydrated in `main.tsx` boot). Mockup affordances the backend can't back yet (avatar photo, member-since, party-count, Edit-profile, device-session list, delete-account) were rendered as honest read-only equivalents and deferred to R10 (see R9-Deferred + R10 "Profile hero backing data"). **R9.11b (Hub)** — `Hub` restyled to `HubHeroNoPip` (default) + `HubListDetail` (alternative), gated on the `hubLayout` pref. New `HubParty` normalized row + `PartyChooser`/`PartyListDetail` sub-components: hero layout = "Continue" gradient card for the most-recent party + "Your other parties" grid; list layout = master list + detail pane; both fall through to the always-present Solo/Create-party/Join `newActions` (Join disabled in local mode). Server mode adds a medallion (initial-avatar → navigates to `/settings`) + welcome; local mode keeps a plain "Your parties" header. **Hub shows only list metadata** (name · role · members · last-active) — per-party item/gold stats need a server aggregate (deferred; see R9-Deferred "Hub per-party stats"). All create/join/open dispatch paths + dialogs preserved. Tests: ThemeField test converted `option`→`radio` queries; +`AppearanceFields.test` (3) +`hubLayout.test` (4); existing Hub tests unchanged (Solo/Create-party present, Join disabled local, no login chrome). web suite 1100; typecheck + lint green.
> - **R9.12 shipped (Orphan screens + modals).** Single commit, done directly in the main thread. **(a) ShopsList** → R9 list language (framed `rounded-lg border shadow-e1` table, `bg-surface-2` header, `Store` icon + row chevron, R9 status pill, dashed empty-state, `Plus` "New shop"); kept at `max-w-5xl` (Shop-detail family, NOT 6xl — see width note below). **(b) CharacterForm** → ported the `character/CharacterFormGrouped` mockup: grouped sub-sections (Identity / Class & level / Ability) via a local `Group` helper (framed `surface-2` block + `font-display` icon eyebrow), STR helper text; same RHF+Zod schema + field labels (Hub create dialogs only — `EditCharacterDialog` has its own independent form, the roadmap line's "+ EditCharacterDialog" was inaccurate). **(c) Auth flow** (`Login`/`LoginEmail`/`LoginEmailVerify`/`LoginDisplayName`) → new shared `components/auth/AuthShell` (centered `surface` card, e2 elevation, `Swords` brand lockup, `font-display` title); no design-lab mockup existed so derived from tokens + the Settings framed-card idiom; OTP/Discord/display-name flows unchanged. **(d) Modal consistency** → `font-display` on every `DialogTitle` across all 18 dialog consumers (icon+title headers kept their `flex items-center gap-2`, just appended `font-display`). **(e) Dead-code** → deleted unused `stash/StashSearchInput`. **Bonus (stash tables):** the shared `StashItemsTable` now hides the **State** column outside the carried Inventory (equip/attune/charges are Inventory-only §3.4) and surfaces the party-scope action set (Split/Pack/Take-out/Move/Remove) as **inline buttons** instead of the kebab — Inventory (characterId provided) keeps the kebab (too cramped + has equip/attune). Actions column right-flushed (`text-right` + `-mr-2`) to mirror the left-flush Name column. Destructive hover fixed app-wide: inline **Remove** + kebab **Remove**/**Delete character** now hover/focus to a solid `bg-destructive` fill (was a clashing accent-green wash behind red text). **Width review** (all screens vs the tiered scale): ratified stash family + Catalog + Character-Sheet + History as `6xl` (dense tables); everything else `5xl`/narrow — memory updated. Tests: StashItemsTable M5/R1.5 suites rewired from kebab `menuitem` queries to inline `button` queries (no-characterId path). web suite 1100; typecheck + lint green.
> - **R9.13 shipped (Cross-cutting polish).** Three commits (one per thread), all done directly in the main thread. **(a) `useCanDispatch()` wiring** — the offline-multi-member disable affordance (`isServerMode && !online && memberCount >= 2`, §9) now covers the deliberate primary mutation buttons: all stash modals (Create/Rename/Split/Pack/Move/Convert/Drain/SplitEvenly/Transfer), RestRollModal Apply, HomebrewForm submit (×2), DeleteHomebrew/DeleteStash confirms, ShopsList Create, ShopDetail Buy/Sell + Manage Add-stock/Delete, IdentificationPanel batch Identify. Deliberately SKIPPED the per-row micro-controls (StashItemsTable inline +/−/Split/Move/Remove, CurrencyRow ±, CatalogPicker per-row Add) — they're guarded by the store short-circuit + toast, and gating ~26 of them would be noise. **(b) A11y** — the icon-only-button audit came back clean (labels + `aria-hidden` already thorough), so the real gains were: `aria-label` on the four unlabeled data tables (`StashItemsTable` "Items", ShopsList "Shops", CatalogBrowser "Catalog items", ShopDetail "Shop stock"; History already had one), and converting the ShopsList clickable `<tr onClick>` (introduced in R9.12, not keyboard-operable) into a focusable name `<button>` — the only clickable-row in the app. **(c) Empty/loading/error** — mostly already consistent (screen-level empties = framed dashed box; in-card/table empties = `p-4` muted `<p>`; Hub owns the only real loading/error since party screens hydrate pre-render via `PartyScopeSync`). Unified the two genuine inconsistencies: `StashItemsTable`'s search-empty branch was missing the `p-4` its siblings had, and HistoryScreen's `state===null` guard was a bare `<p>` → now the framed dashed box. **Bulk multi-select was explicitly de-scoped** by the user (the roadmap marks it optional). web suite 1100 throughout; typecheck + lint green each commit.
...
- [x] **User-initiated email change with dual-OTP confirmation.** Today `User.email` (OUTLINE §4 line 245) is set at signup time (or when a Discord user adds a backup login) and there is no user-facing surface to change it afterwards. Add a "Change email" flow in Settings that verifies both the **current** email and the **new** email via the existing 8-digit OTP infrastructure before committing the swap.
  ...
  - **Edge cases to design through.**
    ...
    - `newEmail` equals `currentEmail` — reject at request time with `email_unchanged`.
    - User aborts mid-flow — server-side pending-change row is deleted (or ignored via `expiresAt`); no email or `emailVerified` mutation happened.
...
> - **OUTLINE §3.1 amendment needed at kickoff.** The "backup login" language in §3.1 covers ADDING an email to a Discord account; it does not describe changing an already-set email. Draft an amendment before writing code. **DONE 2026-07-15** — §3.1 gained a "Changing an already-set email (R10.1)" bullet under the account-linking block (dual-OTP shape, old-address release, not-in-TransactionLog note).
> - **R10.1 shipped (2026-07-15) on `feat/r10`.** Dual-OTP email change with the **3-collapsed-endpoint** shape (chosen over the 7-endpoint sketch — less surface, mirrors the login request/verify pairing).
>   - **Server.** `PendingEmailChange` Prisma model (mirror of `PendingDiscordLink`; `@@unique([userId])` = one in-flight change per user, upsert-on-start invalidates a prior). Migration `20260715141514_r101_pending_email_change` — **additive only** (new table + indexes + FK; a spurious `Character_inventoryStashId_fkey` drop/re-add that `migrate dev` bundled was stripped — Prisma can't express BUG-001's `DEFERRABLE` FK in its DSL so it re-flags it as drift every run). Four routes on `apps/server/src/auth/routes.ts`, all `getSession`-guarded: `/auth/email/change/{start,verify-current,verify-new,abort}`. Codes live in `VerificationToken` under namespaced identifiers (`emailchange-cur:<userId>`, `emailchange-new:<userId>:<newEmail>`), reusing `generateOtp`/`constantTimeEqual`/`checkLockout`/`recordFailedAttempt`/`resetAttempts`/`recordOtpRequest` + `mailService.sendOtp`. Commit swaps `User.email` + re-stamps `emailVerified` + deletes the pending row + reaps old-address `EmailAuthAttempt` rows in one txn. **TTL = 10 min** (`EMAIL_CHANGE_OTP_LIFETIME_MS`, tighter than the 15-min login OTP). Uniqueness re-checked at both start and commit (+ P2002 catch).
>   - **No audit-log entry** (decided with user). `TransactionLog.partyId` is non-nullable — every entry is party-scoped; an account-level email change has no party context, so no `user-email-changed` reducer entry is written (the earlier roadmap text assumed one). Record = server request log + `emailVerified` re-stamp. Kept us out of the log-authority/broadcast/permission machinery entirely.
>   - **Client.** Shared response schemas in `packages/shared/src/schemas/api.ts` (`emailChange{Start,Sent,Commit,Abort}ResponseSchema`); api helpers `startEmailChange` / `verifyCurrentEmailOtp` / `verifyNewEmailOtp` / `abortEmailChange` in `apps/web/src/lib/api.ts`. New `screens/EmailChange.tsx` — 3-step `AuthShell` wizard (new email → current-address code → new-address code) reusing the `input-otp` primitive; commits via `setSession`, `beforeunload` guard while in-flight, Cancel calls `abortEmailChange`. Route `/settings/email/change` registered inside `ProtectedRoute` (authenticated, unscoped, no sidebar). Settings → Account email `Row` gained a "Change" action (server mode + email-set only).
>   - **Tests.** Server: `routes.email-change.test.ts` (10 integration tests, real Postgres) — happy path + old-`EmailAuthAttempt` reap, 401, `no_current_email`, `email_unchanged`, `email_already_linked` (start + commit-race), 5-strike lockout, `current_not_verified`, multi-tab upsert, abort. Client: `EmailChange.test.tsx` (3 — full 3-step walk + `setSession`, Cancel→abort, Discord-only fallback). Added jsdom shims to `apps/web/src/test/setup.ts` (`ResizeObserver` + `document.elementFromPoint`) that `input-otp` needs — consistent with the existing matchMedia/pointer-capture/Web-Locks shims. Server suite 241→251; web 1100→1103; typecheck + lint green across the workspace.
>   - **Settings entry-point test intentionally skipped** — the "Change" button is a one-line `navigate('/settings/email/change')`; wiring a server-mode session harness onto `Settings.test.tsx` (its only server-mode test) for a single navigate isn't worth it per CLAUDE testing guidance (pragmatic > coverage-chasing). The substantive flow is covered by `EmailChange.test.tsx`.
>   - **Dev-DB caveat (not prod-affecting):** the migration SQL was edited *after* its first `migrate dev` apply, so the dev DB's recorded checksum is stale (`migrate status` doesn't flag it). Prod / fresh checkouts apply the current file cleanly — verified via `migrate deploy` on the `:5434` test DB. A `prisma migrate resolve` on dev would reconcile the checksum if ever wanted.
...
- [x] **Hub per-party stats** (promoted from R9-Deferred 2026-07-15; surfaced during R9.11b — the `HubHeroNoPip` / `HubListDetail` mockups show per-party item + gold counts on each card). The Hub deliberately renders **only available list metadata** (name, role, member count, last-activity) — the item/gold totals live inside each party's full `AppState`, which the Hub does NOT fetch (the whole state per party is too large to pull for a listing screen). To surface them, add a lightweight server aggregate to `GET /sync/parties` (e.g. `itemCount` + `totalCp` denormalized per membership, or a cheap COUNT/SUM join) and extend `PartyListItem`; local mode can compute from the keyed Dexie blob it already enumerates. Until then the card subtitle is role · members · last-active only.

#### R10.3 — Notes

> - **R10.3 shipped (2026-07-15) on `feat/r10`.** Per-party **item count** + **total gold** now render on every Hub card (both hero + list-detail layouts).
>   - **Decisions (with user):** item stat = **total item quantity** (10 arrows = 10, not stack count) across **all** the party's stashes (every character inventory + Storage + Party Stash + Recovered Loot; shop stock is a separate `ShopStockEntry` table, naturally excluded). Gold carried on the wire/blob as **integer `totalCp`** (SECURITY §3.2 CP-integer only); the client divides to gp for display.
>   - **Schema.** `partyListItemSchema` (`packages/shared/src/schemas/api.ts`) gained required `itemCount` + `totalCp` (int, nonneg).
>   - **Server.** `GET /sync/parties` (`apps/server/src/sync/routes.ts`) extends its per-party `Promise.all` with a stash query. **Two-arm gotcha:** `Stash.partyId` is set only for `party`/`recovered-loot` scopes — character stashes have `partyId=null` and belong via `ownerCharacter.partyId`. The query ORs both arms (`{ partyId } | { ownerCharacter: { partyId } }`) so character inventories are counted; then sums `item.quantity` + `currency.toCopper()` (reused from `@app/rules`). Single extra query per party (tiny cardinality — same note as the existing memberCount/lastActivity queries).
>   - **Client.** `Hub.tsx` — `HubParty` +`itemCount?`/`totalCp?`; server mode threads them from `listParties()`, local mode computes from the keyed Dexie blob's `items[].quantity` + `currencies[].toCopper` during the enumeration it already does. Module-level `partyStats()` helper (icon row: `Package N items` · `Coins X gp`) rendered on the hero Continue card, the hero other-parties grid, and the list-detail pane. Gold shown as **gp** (`totalCp / 100`, ≤2 dp).
>   - **Tests.** Server `parties.test.ts` +1 (two-arm aggregate over a character inventory stash: 7 items + 250 cp) + itemCount/totalCp=0 assertions on the fresh-party test. Client `Hub.test.tsx` +2 (local-blob compute: 5 items / 12.34 gp, and empty 0/0). No migration (read-only aggregate). server 241→242; web 1105→1107; typecheck + lint green.
>   - **OUTLINE:** no amendment — the parties-list is an API DTO, not part of the §4 `AppState` model (like the existing `memberCount`/`lastActivityAt`, which §4 also doesn't document). The `packages/shared` schema is the DTO source of truth.

#### R10.4 — Settings account/profile features

- [x] **Settings account features from the mockup that the app doesn't support yet** (promoted from R9-Deferred 2026-07-15; surfaced during R9.11a — the `SettingsProfileHero` mockup shows affordances beyond the current backend). R9.11 renders honest read-only equivalents (initial-avatar hero, read-only Account rows, single-device Logout, local-data Wipe) and defers the real support here. Concrete plan is the **R10 Notes "Profile hero backing data + Edit profile" candidate sibling** below (avatars, member-since, party-count, Edit-profile flow, device sessions, delete-account) — that Notes block is the detailed scoping for this sub-slice.

#### R10.4 — Notes

> - **R10.4 shipped (2026-07-16) on `feat/r10`.** Scope decided with user: **member-since + party-count + change-display-name + device sessions + account data export + delete-account**. **Avatars explicitly deferred to a new R11** (custom upload needs a blob store + type-allowlist + sharp re-encode + size/dimension caps + serving path + a SECURITY amendment — too heavy for this slice; the hero keeps the initial-letter medallion). Delete-account is a **soft delete** (decided with user after a blocker analysis — see below).
>   - **Soft-delete over hard-delete (the key decision).** Hard-deleting `User` is blocked by three `RESTRICT` FKs that survive "leave all parties": `Party.ownerUserId` (archived owned parties keep the row), `Character.ownerUserId` (resolved by the leave cascade), `TransactionLog.actorUserId` (log entries in surviving multi-member parties). Soft-delete keeps the `User` row so **none** break — no FK migration, no tombstone, no cascade-ordering hazard; matches the codebase's `leftAt`/`archivedAt` idioms. Deletion = leave every party (reusing the §8.3 machinery) → anonymize `displayName='[deleted user]'`, null `email`/`emailVerified`/`discordId` (releasing the UNIQUE addresses for reuse), stamp `deactivatedAt`, delete all Session+Account+pending rows, clear the cookie. Login is blocked automatically (no session, no credentials). Sole-DM of a multi-member party → `sole_dm_must_transfer_first` (422), no mutation. See OUTLINE §8.4 + SECURITY §7.3.
>   - **Schema.** `User.deactivatedAt DateTime?` + migration `20260716074026_r104_user_deactivated_at`. **Two migration-tail gotchas handled:** (1) Prisma's `migrate dev` bundled a spurious `Character_inventoryStashId_fkey` drop/re-add that reverts BUG-001's `NO ACTION … DEFERRABLE` to `RESTRICT` — **stripped** (the migration is the additive column only); verified `condeferrable/condeferred/confdeltype='a'` intact post-apply. (2) The §1.2 auth-present CHECK (`discordId IS NOT NULL OR emailVerified IS NOT NULL`) would reject a credential-less soft-deleted row, so the migration **drops + re-adds** it as `deactivatedAt IS NOT NULL OR discordId IS NOT NULL OR emailVerified IS NOT NULL`. Dev DB was `migrate reset` (R10.1 stale-checksum drift + the amended file); prod/fresh checkouts apply cleanly via `migrate deploy` (verified against the test DB).
>   - **Server.** New `apps/server/src/users/routes.ts`: `POST /users/me/display-name` (pure rename, distinct from `/auth/email/set-display-name`), `GET /users/me/sessions` (Session rows; current flagged by matching `su.session.sessionToken`; `createdAt` derived as `expires − 30d` since Session has no createdAt column), `POST /users/me/sessions/revoke` (`{sessionId}` or `{allOthers:true}`; can't revoke current — that's Logout), `GET /users/me/export` (one envelope per active party), `POST /users/me/delete`. Extracted `leavePartyForUser` (`apps/server/src/parties/leave.ts`) from the leave route so both callers share the archive-vs-cascade branch. `createdAt` threaded onto the session-user payload (Auth.js `callbacks.session` + `adapter-overrides.dbToAdapterUser` + the three email-OTP route responses) for the member-since stat.
>   - **Client.** `api.ts` helpers (`updateDisplayName`/`listSessions`/`revokeSession`/`revokeOtherSessions`/`exportAccount`/`deleteAccount`); `session.ts` gained `deleteAccount()` (teardown mirrors `signOut`). `Settings.tsx`: hero member-since + party-count (server → `listParties().length`, local → `listKnownPartyIds().length`), Account display-name Edit dialog (RHF+Zod → `setUserPatch`), Sessions device list + Revoke + "Sign out other devices", account danger zone (Export my data via `triggerDownload` + Delete account confirm; on `sole_dm_must_transfer_first` toasts + navigates to that party's settings). Avatar display NOT added (R11).
>   - **Tests.** Server `users/routes.test.ts` (16 integration, real Postgres) — display-name (happy/401/400/409), sessions (list-flags-current / revoke / can't-revoke-current / can't-revoke-others' / allOthers), export (bundle / 401), delete (**solo→archive+soft-delete+cookie-clear**, **multi-member player→soft-delete + actor-FK-still-resolves invariant**, **sole-DM→422 no-mutation**, **released-email→fresh-signup-new-id**, 401). Client `Settings.r104.test.tsx` (7 — hoisted `isServerMode:true` + api mocks; hero stats, display-name edit, sessions list+revoke, export download, delete→redirect, sole-DM→party-settings). server 258; web 1114; typecheck + lint green.
>   - **Docs:** OUTLINE §8.4 (new — account deletion), §5.17 Settings expanded; SECURITY §7.2 (account export) + §7.3 (account deletion) + §1.2 CHECK amendment noted.

#### R10.5 — Item wishlist per character

- [x] **Item wishlist per character (DM hints)** (promoted from Future/Stretch 2026-07-15). A per-character list of desired items the player is hoping for; the DM sees it as a hint when generating hoards / distributing loot. **New product scope** — needs an OUTLINE data-model entry at charter time (either `Character.wishlist` on the existing schema — `packages/shared/src/schemas/character.ts` — or a dedicated `Wishlist` entity) plus a `wishlist-*` reducer action + matching `TransactionLog` type per the "reducer-action 1:1 with log-type" rule. Surfaces on the Character Sheet (owner edits) + the DM Hoard Generator / Loot Distribution wizards (read-only hint).

#### R10.5 — Notes

> - **R10.5 shipped (2026-07-16) on `feat/r10`.** Charter decisions (with user): **edit = owner + DM/solo**; **entries = both catalog items AND free-text wishes**; **DM hint = Loot Distribution wizard + a new DM Command Center tile → party-wide Wishlist Overview screen**; **both actions audit-logged**.
>   - **Data shape.** A wishlist entry is a tagged union `{ id, kind:'catalog', definitionId } | { id, kind:'text', text }` (`wishlistEntrySchema`), stored as `Character.wishlist Json @default("[]")` (structured → JSON, not the flat `tags String[]`; precedent `ItemInstance.conditionOverrides`). Client-minted `id` per entry so `wishlist-remove` targets one unambiguously (free-text has no key + dups allowed). Free-text is plain text ≤200 chars (SECURITY §4). `characterSchema.parse` gives `[]` by default so pre-R10.5 blobs parse.
>   - **Actions (1:1 action↔log↔metadata).** `wishlist-add` (`{characterId, entry}`) + `wishlist-remove` (`{characterId, entryId}`) across `action.ts` + reducer `types.ts` union + `actionMetadata.ts` (`broadcastOnApplied: true` both — party members see live changes) + `transactionLog.ts` (`wishlist-add` logs `{characterId, entryId, kind, label}` where `label` = item name / wish text so History needs no catalog lookup; `wishlist-remove` logs `{characterId, entryId}`). Reducer `wishlistAdd`/`wishlistRemove`: reject unknown character / duplicate id / unknown catalog definitionId / missing entryId. §8.1 guard = owner-or-DM (mirrors `delete-character`) — DM editing another player's wishlist is allowed + logged.
>   - **Persistence.** `Character.wishlist Json` + migration `20260716124548_r105_character_wishlist` — **stripped the auto-generated BUG-001 FK drop/re-add** (Character-table migration; verified `condeferrable/condeferred/confdeltype='a'` post-apply — see the new memory + schema.prisma DRIFT WARNING). Persistor `persistWishlistAdd`/`persistWishlistRemove` parse the Json through `wishlistEntrySchema` at the boundary. `mappers.fromPrismaCharacter` + `restore.ts` thread `wishlist` (lossless JSON round-trip). state-loader returns it on pull.
>   - **UI.** **Character Sheet** — a **Wishlist button below the encumbrance bar** (chosen with user over a header button / rail card / own page) opens `WishlistDialog`: ItemPicker for catalog + a free-text input; remove buttons; `canEditCharacter` (owner/DM/solo) gates edit affordances, `useCanDispatch` gates the offline-multi-member window. **Loot Distribution wizard** — a `wishlistByDef` map badges any rolled item row whose `definitionId` matches a character's catalog wishlist ("Wished by {names}", read-only). **DM Command Center** — new **Wishlists** tile → new `WishlistOverview` screen (`/party/:partyId/wishlists`, DmOnlyRoute): party-wide read-only aggregate, catalog items with rarity pills + free-text wishes, empty state.
>   - **`summarizeLogEntry`** gained cases for the two log types (History rows read "Added \"X\" to {char}'s wishlist" / "Removed an item from {char}'s wishlist").
>   - **Tests.** rules `wishlist.test.ts` (7 — add catalog/text, unknown char, dup id, unknown def, remove, missing entryId). server `sync/wishlist.test.ts` (5 — free-text round-trip + log, remove, DM-edits-other allowed, player-edits-other rejected, wishlist on `/sync/state` pull). web `CharacterSheet.test` (+2 dialog add/remove), `WishlistOverview.test` (3 — aggregate, empty, unloaded redirect). Loot-badge left to the tested derived state (pragmatic — pure render off already-covered wishlist state). rules 257, server 263, web 1119; typecheck + lint green. Fixture literals across shared/rules/web got `wishlist: []` (schema now requires it).
>   - **Docs.** OUTLINE §4 `Character.wishlist` + the two log types + §8.1 permission rows (add/remove own = owner+DM; view = all).



#### R10.6 — DM bulk "level up party" from the Dashboard

- [x] **DM bulk level-up all characters from the Command Center** (new 2026-07-15). A DM Dashboard control that increments **every** party character's `level` by 1 (capped at 20) in one confirmed action. Reuses the existing **`edit-character`** action (`level` is already in its `changedFields` enum — `packages/shared/src/schemas/action.ts` + `transactionLog.ts`; `broadcastOnApplied: true`), dispatched per-character via the R8.5 batched / continue-on-failure pattern the Loot Distribution Wizard already uses — **no new action variant or log type**. Emits one `edit-character` log entry per character (audit trail preserved per OUTLINE §8; server re-validates DM identity from the session cookie per SECURITY §6). Lands as a tile/button on the DM Command Center (`apps/web/src/screens/DmDashboard.tsx`), `DmOnlyRoute`-gated + `useCanDispatch()`-gated, behind a confirm dialog (bulk mutation). **New UX scope** → note an OUTLINE §5/§8 mention at charter time; the underlying mutation is already spec'd.

#### R10.6 — Notes

> - **R10.6 shipped (2026-07-16) on `feat/r10`.** Smallest R10 sub-slice — no schema, no new action/log type, entirely client + docs.
>   - **Control.** A **"Level up party" button** in the DM Command Center's "Party at a glance" section header (chosen over a launcher tile — tiles navigate to routes; this is an in-place confirmed mutation). `DmOnlyRoute`-gated (inherited) + `useCanDispatch()`-gated (offline-multi-member window) + behind an `AlertDialog` confirm. The button is **hidden when every character is already level 20** (`levelableCount === 0`).
>   - **Dispatch.** Reuses `edit-character` `{ characterId, patch: { level: c.level + 1 } }` per character, skipping any at level 20 (the cap). Batched via the R8.5 pattern lifted from the Loot Distribution wizard: build a `pending[]` of `dispatch(..., { onRejection: () => {} })` promises, `Promise.all`, tally `ok` + per-failure labelled `toast.error`, one summary `toast.success`. Continue-on-failure — a rejected character doesn't abort the rest. One `edit-character` log entry per character (audit trail per OUTLINE §8; server re-validates DM identity per SECURITY §6). The 20-cap is enforced client-side by skipping maxed characters; the reducer's `edit-character` no-op guard also rejects a same-level patch.
>   - **Tests.** `DmDashboard.test.tsx` +2 (levels Alice 5→6 + Bob 3→4 after confirm; button hidden when the sole character is level 20). web 1119→1121; typecheck + lint green.
>   - **Docs.** OUTLINE §5.9 gained the level-up control mention. No §8 matrix row needed — the existing "Edit own character … level — DM (any character, via explicit action)" row already authorizes it; bulk is just that action applied N times.
...
>   - **Profile hero backing data + Edit profile** (surfaced R9.11a — the Settings `SettingsProfileHero` mockup needs data the app doesn't store yet). To render the hero faithfully (avatar photo + "N parties" + "Member since {date}" + an Edit-profile flow) we need:
>     - **Profile pictures / avatars** — no avatar storage today (the hero shows the display-name initial). Needs an `avatarUrl` (or blob) on `User`, an upload/crop flow (the mockup's camera pip), a serving path, and a fallback to the initial. Note SECURITY implications: uploaded images are user-controlled binaries (size/type validation, no SVG/script, virus-scan or re-encode). Discord accounts already have `avatarUrl` per OUTLINE §3.1 — could seed from Discord as the default before custom upload lands.
>     - **Account "member since"** — `User` carries no `createdAt` exposed to the client. Add it to the schema + `SessionUser` so the hero can show "Member since {month year}".
>     - **Party count in Settings** — the hero's "N parties" stat. Server mode already has `GET /sync/parties` (used by the Hub); Settings would fetch it (or the session payload could carry a count). Local mode has no multi-party concept, so hide the stat there.
>     - **Edit profile flow** — the hero's "Edit profile" button + the Account rows' inline Edit/Change. Bundles display-name change (see below) + avatar upload; email change is the separate R10.1 dual-OTP slice. Until this lands, the R9.11 hero is read-only (initial + name + email, no Edit button, no stats).
...
>   - **Manage linked accounts** — Settings → Linked accounts already lets a user add Discord OR email as a login method (`apps/web/src/components/auth/LinkedAccounts.tsx`; R9.11a restyled it with green "Connected" badges). UX consolidation opportunity: unify with the new Account section, add unlinking with the "must have at least one login method" invariant enforced.
>   - **Session management** — "Sign out of other devices." Today the session cookie is single-device implicit (Auth.js `Session` model at `apps/server/prisma/schema.prisma:417`); R9.11a's Settings "Sessions" section is just the single-device Logout. Add a session-listing UI that surfaces `Session` rows for the current user with per-row revoke buttons (matches the mockup's device list).
>   - **Self-service data export + delete account** — a GDPR-adjacent "Download all my data as JSON" button (distinct from per-party JSON export in §3.13), followed by a "Delete my account" flow (the mockup's Danger-zone "Delete account", not built — R9.11a's Danger zone is the local-data Wipe) that cascades to `PartyMembership` (marks leftAt), `Character` (delete-character cascade), user-owned homebrew (surface a "reassign to party" or "delete" prompt per row), and finally the `User` row. Discord token is already not persisted per SECURITY §1.1. SECURITY §7 backup discussion is the natural place to draft the spec.
...
### R11 — Custom profile avatars (stub — deferred from R10.4)

Custom avatar upload, split out of R10.4 (2026-07-16) because it's the single heaviest/riskiest item in the profile-features set and stands alone. Today the profile hero shows the display-name initial; Discord accounts carry an `avatarUrl` (Discord CDN string) that isn't even displayed yet.

- [ ] **Custom avatar upload + serving.** Let a user upload a profile picture (the `SettingsProfileHero` mockup's camera pip). New scope in every direction:
  - **Storage.** No blob store exists (Postgres + Dexie only). Add a backend — filesystem volume, S3-compatible bucket, or a `bytea` column — each with its own backup/serving story. Serve via a `GET /users/:id/avatar` route (or static path) with cache headers + a fallback to the initial when absent.
  - **Security (SECURITY.md amendment required before code).** Uploads are user-controlled binaries: **re-encode server-side** (decode → re-emit PNG/WebP via sharp) to strip embedded script/metadata; **raster-only allowlist**, never trust extension/MIME; **SVG is forbidden** (script vector → stored XSS if served inline); enforce **max byte size + max dimensions before decode** (pixel-flood/decompression-bomb DoS) + a decode timeout; drop EXIF/GPS on re-encode; serve from a path with strict `Content-Type` + `Content-Disposition` (or a separate origin). See the deferral rationale in the R10.4 avatar Q&A.
  - **Client.** Upload + crop flow (the mockup's camera pip), preview, and the hero/medallion/nav consuming the served URL with an initial-letter fallback.
  - **Cheaper interim (could ship first, or as part of this slice's step 1):** display the existing Discord `avatarUrl` for Discord accounts (already stored — just render it) with the initial-letter fallback for email accounts. No upload, no new storage — visual parity for Discord users with none of the binary-handling risk.
  - **Schema.** `User.avatarUrl` already exists (Discord CDN string). Custom uploads either overload it with a served path or add a distinct `avatarBlobKey` — decide at charter time. `SessionUser.avatarUrl` is already on the wire (R3.x).
...
- [ ] **Initiative tracker (DM combat tool).** A DM Command Center tool for running encounter turn order. Surfaces as a tile on the DM Command Center (`apps/web/src/screens/DmDashboard.tsx`), `DmOnlyRoute`-gated. Would also help fill the Command Center's tile grid (currently 5 tiles in a 4-col layout leaves 3 empty slots on wide screens — surfaced 2026-07-16).
  - **Combatant roster.**
    ...
    - **Bulk roll** — a "Roll all" action rolls every row that has a roll mode + modifier (i.e. the monster/NPC rows, and any PC row the DM opts to auto-roll), respecting each row's adv/normal/dis. PC rows with a manually-entered value are left untouched.
  - **Turn cycle.**
    - **Order by initiative** — after rolling, sort the list descending by initiative value (ties: keep stable / DM can reorder manually; decide tie-break at charter — 5e uses a dex/DM-decides tiebreak).
    - **Current-turn highlight** — the active combatant's row is highlighted in the **accent color**. Rows sharing the same initiative value are highlighted together (simultaneous turns).
    - **End turn** — advances the highlight to the next combatant (next distinct initiative value), wrapping to the top + incrementing a **round counter** at the end of a cycle.
    - **Remove from cycle** — the DM can drop a combatant (defeated / fled) mid-encounter; the turn pointer adjusts so removing the current combatant advances correctly and doesn't strand the highlight.
  - **Data-model decision (charter time).** Encounter/combat state is **transient table-side state**, not inventory mutation, so it lives OUTSIDE the audit-logged `TransactionLog` model. Simplest: an **ephemeral client-only store** (Zustand, not persisted — the tracker resets on reload, which is acceptable for a live-table tool). If cross-device/live-sync (DM on laptop, view on tablet) or reload-survival is wanted, escalate to a lightweight `Encounter` entity (persisted, possibly broadcast) — bigger scope; default to client-only for v1. **Dice rolling is pure** — reuse/extend a shared roller (cf. `packages/rules` charge-roll helpers) so adv/dis + modifier logic is unit-testable. Optional later: per-combatant HP tracking ties into the "Light character sheet expansion" HP field if that lands first.
- [ ] **Expand gems & art valuables (hoard roll ↔ seed catalog fidelity).** Today `hoard.ts` rolls a single gem ladder (`GemTier = 10 | 50 | 100 | 500 | 1000 | 5000`) but the DMG treats **gemstones** and **art objects** as two separate value ladders (gems: 10/50/100/500/1,000/5,000 gp; art: 25/250/750/2,500/7,500 gp). Two concrete gaps: (1) the seed catalog has no **10 gp** or **50 gp** gems, so a low-CR hoard's "pick a 10/50 gp gem" rows are un-pickable; (2) the seed carries art-tier valuables (25/250/750/7,500 gp — Bone Scroll Tube, Silver Chalice, Gold Mask, Jeweled Gold Crown) that the roll's gem-only ladder can never target. Scope: split the roll into gem + art ladders (or at minimum add the missing 10/50 gp gems to the seed so every rolled tier is fulfillable), and broaden the seed gem/art coverage. Loot-wizard row labels already distinguish "gem" — extend to "art object" when the split lands. See `packages/rules/src/hoard.ts` `GEM_TABLES` + `packages/seeds/data/dmg-2024.json`.
