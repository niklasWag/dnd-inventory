...
- [ ] **R9.7 — Shop (Storefront + DM manage).** Both surfaces; ItemPicker rail for add-stock.
...
- [ ] **R9.12 — Cross-cutting polish.** Wire `useCanDispatch()` on all primary Save buttons (R5.1.d carryforward); accessibility pass (keyboard nav, ARIA on icon-only buttons, WCAG AA contrast, screen-reader on Character Sheet + Party Stash); optional bulk multi-select on stash tables; empty/loading/error states.
