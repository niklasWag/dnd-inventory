...
- After login, the user lands on a **hub** with three actions:
  1. **Create a party** — user becomes that party's DM. The flow prompts: *"Do you also play a character in this party?"* (yes / no, default yes). If yes, a player slot + character is created alongside the DM membership; if no, only the DM membership is created and the user can add a player membership later. So a single user is both DM and player by default, but it is an explicit choice rather than silent auto-add.
