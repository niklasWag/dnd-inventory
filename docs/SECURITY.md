...
- **SMTP misconfiguration guard:** at server startup, if `SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASS` / `SMTP_FROM` env vars are absent or incomplete, email auth is **disabled entirely** — the email login UI is hidden and the endpoint returns `503`. This prevents silent failures where SMTP is broken and users receive no code but see no error.
