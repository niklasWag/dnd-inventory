...
**Optimistic UI and terminal signals (R8.5).** "Optimistic UI" covers the immediate local reducer apply and an optional interim **"Queued…"** toast — but NOT a terminal *success* signal. A green "success" toast must not fire the instant the local reducer accepts an action, because the mutation is not yet terminal: in server mode it still has to survive `POST /sync/actions`, where a §2.1 guard may reject it (the BUG-005 class — the green toast flashing before the red rejection). The terminal success signal therefore waits for the **server ack** (server mode — the sync queue resolves a `MutationOutcome` after the round-trip) or the **local-mode synchronous shim** (local mode has no server, so the local apply *is* terminal). This does not soften server authority — the server still re-validates every mutation and the client rejection is display-only; it only constrains *when* the client is allowed to show success. Enforced in code by the `useDispatch` hook + the `local/no-sync-toast-success-after-dispatch` ESLint rule.
...
- **In server mode, the server is the sole authority for TransactionLog contents.** The client-side reducer emits log slices during optimistic dispatch (so the UI can display in-flight state), but those local entries are treated as ephemeral — after each successful `/sync/actions` batch the client re-reads the canonical log from the server via `/sync/state` (see `apps/web/src/sync/queue.ts` id-minting canonicalisation path) and any local entries are replaced. Reader components (`ItemHistory`, party log, etc.) MUST behave the same for local-mode and server-mode entries, but MUST NOT assume the local entry's id / timestamp / actorRole match the server's — those fields are server-authoritative and only align after the post-batch pull. RH1 will retire the client-side minting of ids that surface this divergence; until then, any UI that filters the log by an entity id it created locally (e.g., filtering by `itemInstanceId` immediately after `acquire`) may miss entries — see BUG-004 for the manifestation.
